...
| **G1 — Secure Data Supply** | Egoric snapshot may be accepted for local/test integration | Dedicated GET capability; PII denial; deterministic quoted ETag/304; method and generic-API denial; non-PII audit; feature flag; independent QA PASS | **Technical QA PASS locally at `repositoryrealms@28ceff6`; publication, PR, merge, and Product Owner acceptance pending** |
| **G2 — Business Memory** | A snapshot may enter the LeozOps analytical read model | Schema fail-closed; immutable/idempotent storage; tenant isolation; no-write-egress; replay tests; full suite/typecheck PASS | Blocked pending G1 publication, merge, and acceptance |
