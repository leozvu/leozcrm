...
| **G5 — Shadow Trust** | Read-only CEO pilot may be released | Independent deployment, production canary, ten business days of read-only shadow, freshness/reliability targets, useful output, no material false claims or workflow regression | **S2.A and local P1 decision tooling accepted; the decision example remains pending, so P1/P2/deployment remain blocked** |
