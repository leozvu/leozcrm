...
export const SUPERVISED_HAND_QUALIFICATION_SCHEMA = 'leozops_supervised_hand_qualification_v1' as const;
...
export interface SupervisedHandQualification {
  ...
  source_contract: 'repositoryrealms.ceo.command';
  source_contract_version: 1;
  endpoint_path: '/api/ceo/v1/commands';
  receipt_path: '/api/ceo/v1/commands/receipts';
  action: 'task.create';
...
export interface RepositoryRealmsTaskCreateEnvelope {
  contract: 'repositoryrealms.ceo.command';
  version: 1;
...
export function validateSupervisedHandQualification(input: unknown): {
  ...
  if (!Array.isArray(root.source_files) || root.source_files.length !== 3) {
    throw new SupervisedActionError('invalid_hand_qualification', 'qualification.source_files must contain three pinned files');
  }
  ...
  const value: SupervisedHandQualification = {
    ...
    source_ref: exactLiteral(root.source_ref, 'main', 'qualification.source_ref'),
    source_commit: sourceCommit,
    command_key: exactLiteral(root.command_key, REPOSITORYREALMS_TASK_CREATE_COMMAND, 'qualification.command_key'),
    source_contract: exactLiteral(root.source_contract, 'repositoryrealms.ceo.command', 'qualification.source_contract'),
    source_contract_version: exactLiteral(root.source_contract_version, 1, 'qualification.source_contract_version'),
    endpoint_path: exactLiteral(root.endpoint_path, '/api/ceo/v1/commands', 'qualification.endpoint_path'),
    receipt_path: exactLiteral(root.receipt_path, '/api/ceo/v1/commands/receipts', 'qualification.receipt_path'),
    action: exactLiteral(root.action, 'task.create', 'qualification.action'),
    scope: exactLiteral(root.scope, 'command.task.create', 'qualification.scope'),
